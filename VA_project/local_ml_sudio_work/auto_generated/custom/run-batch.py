...
def init():
    ...
    g_file_loader_dictionary = {
        ...
        '.tiff': load_image, '.bmp': load_image, '.gif': load_image,
        '.parquet': load_parquet, '.pqt': load_parquet
        }
...
def get_input_schema(model_path):
    try:
        ...
        g_logger.warn("No signature present in MLmodel file: " + ml_model.__str__() +
...
def load_csv(data_file):
    if g_schema and g_schema.is_tensor_spec():
        ...
        g_logger.info("Reading csv input file into pandas dataframe because the model specifies column based input signature or no signature.")
...
def load_image(data_file):
    ...
    if g_schema is None:
        ...
        if len(data_type) == 1 and data_type[0] == DataType.binary:
            ...
            raise TypeError("Scoring image files with a ColSpec signature containing "\
...
def load_parquet(data_file):
    g_logger.info("Reading parquet input file into pandas dataframe.\n" \
        ...
        if isinstance(arguments, pd.DataFrame):
            ...
            for data_file in arguments:
                try:
                    ...
                    data_loader = g_file_loader_dictionary.get(file_extension)
                    if data_loader is None:
                        g_logger.error("Error processing input file: '" + str(data_file) + "'. File type '" + str(file_extension) +"' is not supported." )
                    else:
                    ...
                    g_logger.error("Error processing input file: '" + str(data_file) + "'. Exception:"+ str(e))
...
def run(batch_input):
    try:
        ...
        pred_proba = g_model.predict_proba(batch_input)

        # Convert pred_proba to a NumPy array
        pred_proba_np = np.array(pred_proba)

        # Check for NaN values in pred_proba
        if np.isnan(pred_proba_np).any():
            g_logger.error("NaN values detected in predict_proba output.")
            formatted_result = []
        else:
            # Create a DataFrame to store the prediction result and the prediction probability
            result_df = pd.DataFrame(pred_proba_np)
            result_df[result_df.shape[1]] = predict_result

            # Convert DataFrame to a list of lists
            formatted_result = result_df.to_numpy().tolist()
